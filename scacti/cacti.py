...
def readcacti(wavpath):
    cactipath = os.path.join(os.path.dirname(wavpath), pathlibpath(wavpath).stem) + '.cacti'
    with open(cactipath, "r", encoding='utf-8') as cactifile:
        ...
        # for every line in the file
            ...
            if line.startswith("time="):
                listtime = [float(item) for item in
                        line.replace("time=", "").replace("\n", "").replace("[", "").replace("]", "").split(",")]
            if line.startswith("frq="):
                listfrq = [float(item) for item in
                       line.replace("frq=", "").replace("\n", "").replace("[", "").replace("]", "").split(",")]
            if line.startswith("smo="):
                listsmo = [float(item) for item in
                       line.replace("smo=", "").replace("\n", "").replace("[", "").replace("]", "").split(",")]
